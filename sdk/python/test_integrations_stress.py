...
Tests:
  ...
  2. LlamaIndex adapter — batch ingestion, VectorStoreIndex query, delete
  3. Concurrent stress  — 3 isolated tenants ingesting 1000 docs each in parallel
...
EMBED_DIM         = 64     # lightweight fake embeddings — no OpenAI key needed
STRESS_DOCS       = 1000   # docs per tenant
STRESS_TENANTS    = 3      # concurrent isolated tenants
SEARCH_K          = 5
...
def record(name: str, ok: bool, elapsed: float, detail: str = "") -> None:
    ...
    print(f"\n[{icon}] {name}  ({elapsed:.3f}s){f'  {detail}' if detail else ''}")

def make_tenant(plane: ControlPlane, prefix: str = "sdk-stress") -> tuple[str, DBXClient]:
...
def test_langchain(plane: ControlPlane) -> None:
    ...
    try:
        ...
        # ── 1a: batch ingestion ───────────────────────────────────────────
        texts  = [f"LangChain document number {i} about AI databases" for i in range(STRESS_DOCS)]
        ...
        record("LangChain: add_texts batch ingestion",
               ok, elapsed, f"{throughput:.0f} docs/s  |  {len(ids)}/{STRESS_DOCS} indexed")

        # ── 1b: from_texts class method ───────────────────────────────────
        t0 = time.perf_counter()
        extra_texts = [f"Extra doc {i}" for i in range(50)]
        store2 = DBXVectorStore.from_texts(
            extra_texts, embedder,
            client=client, index_name="lc_idx2"
        )
        elapsed = time.perf_counter() - t0
        ...
        record("LangChain: similarity_search",
               ok, elapsed, f"k={SEARCH_K}, returned {len(docs)} docs")
        if docs:
        ...
        record("LangChain: similarity_search_with_score",
               ok, elapsed, f"scores: {[round(s,4) for _,s in scored[:3]]}")
        ...
        elapsed = time.perf_counter() - t0
        record("LangChain: similarity_search_by_vector",
               len(docs_by_vec) > 0, elapsed, f"returned {len(docs_by_vec)} docs")

        # ── 1f: add_documents ─────────────────────────────────────────────
        from langchain_core.documents import Document
        extra_docs = [Document(page_content=f"Doc {i}", metadata={"n": i}) for i in range(20)]
        t0 = time.perf_counter()
        new_ids = store.add_documents(extra_docs)
        elapsed = time.perf_counter() - t0
        record("LangChain: add_documents", len(new_ids) == 20, elapsed, "20 Document objects")
        ...
        elapsed = time.perf_counter() - t0
        record("LangChain: as_retriever().invoke()",
               len(retrieved) > 0, elapsed, f"{len(retrieved)} docs")
...
def test_llamaindex(plane: ControlPlane) -> None:
    ...
    if not LLAMA_AVAILABLE:
        record("LlamaIndex: SKIPPED", True, 0.0,
    ...
    try:
        ...
        throughput = STRESS_DOCS / elapsed
        record("LlamaIndex: add() batch ingestion",
               ok, elapsed, f"{throughput:.0f} nodes/s  |  {len(returned_ids)}/{STRESS_DOCS} indexed")
        ...
        record("LlamaIndex: query() VectorStoreQuery",
               ok, elapsed,
               f"returned {len(result.nodes or [])} nodes, scores: {[round(s,4) for s in (result.similarities or [])[:3]]}")
        if result.nodes:
        ...
        store2 = DBXVectorStore.from_params(client=client, index_name="li_idx2")
        record("LlamaIndex: from_params() factory", True, 0.001, "instance created")

    except Exception as e:
        record("LlamaIndex: UNEXPECTED ERROR", False, 0.0, traceback.format_exc(limit=3))
    finally:
        teardown(plane, tid)
...
def _tenant_worker(plane: ControlPlane, worker_id: int) -> dict:
    ...
    docs_count = 500
    try:
        texts = [f"Worker {worker_id} document {i} discussing vector isolation" for i in range(docs_count)]
        t0 = time.perf_counter()
...
def test_concurrent(plane: ControlPlane) -> None:
    print("\n" + "="*60)
    print(f"  SECTION 3 — Concurrent stress: {STRESS_TENANTS} isolated tenants × 500 docs")
    print("="*60)
    ...
    for r in sorted(worker_results, key=lambda x: x["worker_id"]):
        if r.get("ok"):
            log(INFO, (
                ...
                f"ingest {r['ingest_s']:.2f}s  |  search {r['search_s']:.3f}s  |  hits={r['hits']}"
            ))
        else:
            log(FAIL, f"Tenant {r['worker_id']} FAILED: {r.get('error','')}")

    record("Concurrent: all tenants isolated and searchable",
           all_ok, total_elapsed,
...
def test_one_line_swap(plane: ControlPlane) -> None:
    ...
    for adapter_name, adapter_cls in [("LangChain DBXVectorStore", DBXVectorStore)]:
        ...
        try:
            # ── This exact same code block works with both Pinecone and DBX ──
            ...
            log(INFO if ok else FAIL, f"{adapter_name}: returned {len(docs)} docs from same code path")
        except Exception as e:
            results_swap.append((adapter_name, False))
            log(FAIL, f"{adapter_name}: {e}")
        finally:
            teardown(plane, tid)
...
def main() -> None:
    ...
    if failed > 0:
        ...
        print(f"  \033[92mAll tests passed! LangChain + LlamaIndex adapters are working.\033[0m")
